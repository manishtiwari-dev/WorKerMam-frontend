            ...
            <div class="media-body">
                <h6 class="mt-0 mb-1">{{ ucwords($note->user->name ?? '') }}
                    <span class="pull-right font-italic font-weight-light"><small>
                            {{ Carbon\Carbon::parse($note->created_at)->format('d-m-Y')}} </small>
                        {{-- @if ($user->cans('edit_job_applications')) --}}
                            <a href="javascript:;" class="edit-note" data-note-id="{{ $note->id }}"><i
                                    class="fa fa-edit ml-2"></i></a>
                            <a href="javascript:;" class="delete-note" data-note-id="{{ $note->id }}"><i
