    ...
    <div class="portlet-body">
        <div class="row font-12">
            ...
            <div class="col-sm-12 col-md-4">
                ...
                <p class="text-muted">{{ ucwords($schedule->job_application->full_name ?? '') }}</p>
            </div>

            <div class="col-sm-12 col-md-4">
                <strong>@lang('app.email')</strong><br>
                <p class="text-muted">{{ $schedule->job_application->email ?? '' }}</p>
            </div>

            <div class="col-sm-12 col-md-4">
                <strong>@lang('app.phone')</strong><br>
                <p class="text-muted">{{ $schedule->job_application->phone ?? '' }}</p>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <p class="text-muted">
                {{ ucwords($schedule->job_application->job->title ?? '') . ' (' . ucwords($schedule->job_application->job->location->location ?? '') . ')' }}
            </p>
        <hr>
        <div class="row">
            <div class="col-sm-6">
...
<div class="modal-footer">
    <button type="button" class="btn dark btn-outline" data-bs-dismiss="modal">@lang('app.close')</button>
</div>
...
<script>
    ...
    function statusChange(status) { 
       ...
       swal({
            title: `Do you want to send mail to candidate or not.`,
            text: "Do you want to send mail to candidate or not.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
    ...
    function statusChangeConfirm(status, mailToCandidate) { 
        ...
        var id = "{{ $schedule->id }}"; 
        $.ajax({
            url: '{{ route('recruit.interview-schedule.change-status') }}',
    ...
    function editSchedule() {
        var url = "{{ route('recruit.interview-schedule.edit', $schedule->id) }}";
