    ...
    <div class="card contact-content-body">
        <div class="card-header">
            <div class="d-flex align-items-center justify-content-between">
                <h6 class="tx-15 mg-b-0">{{ __('sales.income_list') }}</h6>
                <a href="{{ route('sales.income.create') }}" class="btn btn-sm btn-bg">
                    {{ __('sales.add_income') }}</i></a>
            </div>
        </div>
        <div class="card-body">
                ...
                <table class="table  table_wrapper">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('sales.income_type') }}</th>
                            <th>{{ __('sales.amount') }}</th>
                            <th>{{ __('sales.Payer') }}</th>
                        ...
                        @forelse($content->app_transactions as $key => $transaction)
                          @if($transaction->type == 'income')
                            <tr>
                                ...
                                <td>@if(!empty($transaction->customer_details))
                                    {{$transaction->customer_details->first_name}}@endif
                                </td>

                                <td  class=" ml-5">{{ $transaction->payment_method_id == 1 ? 'Online' : 'Offline' }}</td>

                               <td class="d-flex justify-content-center">
                                <a href="{{ route('sales.income.edit', $transaction->id) }}"
    ...
    @push('scripts')
        <script>
            ...
            $(document).on("click", "#delete_btn", function() {
                ...
                $('#deleteExpenseId').val(income_id);
            });
            $(document).on('click', '.expenseDelBtn', function() {
                var income_id = $('#deleteExpenseId').val();
