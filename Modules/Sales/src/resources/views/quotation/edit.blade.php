<x-app-layout>
    ...
    <div class="card">
        <div class="tab-content add-quotation-wrapper">
            <form method="post" class="needs-validation" autocomplete="off"
                action="{{ route('sales.quotation.update', $content->quotation->id) }}" enctype="multipart/form-data"
                novalidate>
                @csrf
                @method('PUT')

                <div class="card-header d-flex align-items-center justify-content-between py-2 px-3">
                    <h6 class="tx-15 mb-0">Update Quotation</h6>
                </div>
                <div class="card-body">
                    <div class="row ">
                        <!-- Customer Start -->
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="quotation_number"
                                    value="{{ $content->quotation->quotation_no }}" required>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control datepicker2" name="quotation_date"
                                    value="{{ \Carbon\Carbon::parse($content->quotation->quotation_date)->format('d/m/Y') }}"
                                    required>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control datepicker1" id="expiry_date" name="due_date"
                                    value="  {{ \Carbon\Carbon::parse($content->quotation->expiry_date)->format('d/m/Y') }}"
                                    required>
                            </div>
                        </div>
                            ...
                            <select class="form-select form-control selectsearch" name="customer_id" id="customer_id"
                                ...
                                @if (!empty($content->customer))
                                    @foreach ($content->customer as $ls_data)
                                        <option
                                            {{ $content->quotation->customer_id == $ls_data->customer_id ? 'selected' : '' }}
                        ...
                        <input type="hidden" value="{{ $content->quotation->address_id }}" id="dbBillingAddsId">
                        <!-- Currency Start -->
                                ...
                                @if (!empty($content->currency))
                                    @foreach ($content->currency as $ls_data)
                                        <option value="{{ $ls_data }}"
                                            @if ($ls_data == $content->quotation->currency) selected @endif> {{ $ls_data }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                                ...
                                @if (!empty($content->TaxGroup))
                                    ...
                                    @foreach ($content->TaxGroup as $taxgrp_data)
                                        <option value="{{ $taxgrp_data->tax_group_id }}"
                                            {{ $content->quotation->tax_group_id == $taxgrp_data->tax_group_id ? 'selected' : '' }}>
                        ...
                        <div class="col-lg-4 col-sm-5 my-2 my-md-0">
                            ...
                            <select class="form-select form-control selectsearch" name="tax_type" id="price_enabled">
                                ...
                                <option value="0" {{ $content->quotation->tax_type == 0 ? 'selected' : '' }}>
                                    {{ __('crm.yes') }}</option>
                                <option value="1" {{ $content->quotation->tax_type == 1 ? 'selected' : '' }}>
                                    {{ __('crm.no') }}</option>
                            </select>
                            <div class="invalid-feedback">
                    ...
                    @if ($content->invoice_type == 'products')
                                    ...
                                    <table class="table table_wrapper create_table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-center tax_outer" style="width: 15%;">
                                                    {{ __('crm.tax') }}
                                                </th>
                                                <th class="text-center" style="width: 15%;">
                                                    Total Amount
                                        ...
                                        <input name="test" type="hidden" class="form-control addsub_total"
                                            id="sub_total" placeholder="" value="0" required>
                                        <tbody id="add">
                                            @if (!empty($content->quotation))
                                                @foreach ($content->quotation->crm_quotation_item as $key => $quoteItem)
                                                    <tr class="addmore">
                                                        <td colspan="8">
                                                            <table>
                                                                <tr>
                                                                    <td style="width:20%;">
                                                                        ...
                                                                        <input name="item_name[]" type="text"
                                                                            ...
                                                                            value="{{ $quoteItem->item_name ?? '' }}" required>
                                                                        <div class="invalid-feedback">
                                                                    ...
                                                                    <td class="text-center " style="width:10%;">
                                                                        <input name="quantity[]" type="number"  class="form-control quantity"
                                                                            id="quantity_{{ $key + 1 }}"  placeholder="{{ __('crm.qty') }}" value="{{ $quoteItem->quantity ?? 1 }}" onblur="calculation({{ $key + 1 }})" required>
                                                                            <div class="invalid-feedback">
                                                                    ...
                                                                    <td style="width:15%;">
                                                                        <input name="discount[]" type="text"
                                                                            ...
                                                                            value="{{ $quoteItem->discount ?? 0 }}">
                                                                    </td> 
                                                                    <td style="width:15%;">
                                                                        <input name="taxable_amount[]" type="text" class="form-control" id="taxable_amount_{{ $key + 1 }}" onkeyup="" placeholder="  {{ __('crm.taxable_amount_place') }}" value="{{ $quoteItem->item_cost }}">
                                                                    </td>
                                                                    <td class="text-center tax_outer" style="width:15%;">
                                                                        <select
                                                                            class="form-select form-control tax selectsearch tax_dropdown"
                                                                            onchange="calculation({{ $key + 1 }})"
                                                                            name="tax_group_id[]" id="tax_{{ $key + 1 }}" style="width:100px">
                                                                        ...
                                                                        @php 
                                                                            $tax_string ='';
                                                                            if(!empty($quoteItem->tax_component)){
                                                                                $tax_compant_array = json_decode($quoteItem->tax_component);

                                                                                foreach($tax_compant_array as $taxAry){
                                                                                    foreach($taxAry as $taxKey => $taxValue){
                                                                                        $tax_string .= $taxKey . " : ". $taxValue->Value."  ";
                                                                                    }
                                                                            ...
                                                                            @endphp
                                                                        <div id="taxcomponent_{{ $key + 1 }}">{{$tax_string}}</div>
                                                                    ...
                                                                    <td class="text-center" style="width:15%;">
                                                                        <input name="item_cost[]" type="text"
                                                                            ...
                                                                            value="{{ $quoteItem->final_amount ?? 0 }}" readonly
                                                                            required>
                                                                        <div class="invalid-feedback">
                                                                            {{ __('crm.amount_error') }}
                                                                        </div>
                                                                                ...
                                                                                <div class="row g-3 align-items-center">
                                                                                    <div class="col-auto d-flex gap-2 mb-2">
                                                                                        ...
                                                                                        <label for="staticEmail"
                                                                                            class="col-form-label">{{ $attribute->option_name }}</label>
                                                                                        ...
                                                                                        <select
                                                                                            ...
                                                                                            @foreach ($attribute->option_value_list as $optionvalues)
                                                                                                ...
                                                                                                <option
                                                                                                    value="{{ $attribute->options_id }}_{{ $optionvalues->options_values_id }}"
                                                                                                    {{ $selected }}>
                                            ...
                                            @else
                                                <tr class="addmore">
                                                    <td>
                                                        ...
                                                        <input name="item_name[]" type="text" class="form-control" id="itemName_1"
                                                            placeholder="{{ __('crm.item_name') }}" value="" required>
                                                        <div class="invalid-feedback">
                                                            {{ __('crm.item_name_error') }}
                                                        </div>
                                                        <input type="hidden" name="item_id[]">
                                                    ...
                                                    <td class="text-center ">
                                                        <input name="quantity[]" type="number" class="form-control quantity" id="quantity_1"
                                                            placeholder="{{ __('crm.qty') }}" value="" onblur="calculation(1)" required>
                                                        <div class="invalid-feedback">
                                                            {{ __('crm.quantity_error') }}
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td class="text-center">
                                                        <input name="discount[]" type="number" class="form-control discount" id="discount_1"
                                                    ...
                                                    <td>
                                                        <input name="taxable_amount[]" type="text" class="form-control taxable_amount"
                                                            id="taxable_amount_1" onkeyup="" placeholder="Enter Sub Total">
                                                    </td>
                                                    <td class="text-center tax_outer">
                                                        <select class="form-select form-control tax selectsearch tax_dropdown"
                                                            onchange="calculation(1)" name="tax_group_id[]" style="width:100px" id="tax_1">
                                                    ...
                                                    <td class="text-center">
                                                        <input name="item_cost[]" type="text" class="form-control amount" id="amount_1"
                                                            placeholder="{{ __('crm.amount') }}" value="" readonly required>
                                                        <div class="invalid-feedback">
                                                            {{ __('crm.amount_error') }}
                                                        </div>
                                                    </td>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center ">
                                                    <input name="sub_total" id="finalUnitPrice" type="text" class="form-control"
                                                        value="{{ $content->quotation->subtotal }}" readonly required>
                                                </td>
                                                <td>
                                                    <input name="total_Descount_price" type="number" id="total_Descount"
                                                        class="form-control totalDiscount" onkeyup="totalDisc()"
                                                        value="{{ $content->quotation->discount ?? 0 }}" readonly required>
                                                </td>
                                                <td>
                                                    <input name="total_taxable_amount" id="total_tax_amount" type="number"
                                                        class="form-control" value="{{ $content->quotation->taxable_value }}" readonly
                                                        required>
                                                </td>
                                                <td class="text-center tax_outer">
                                                    <input name="totaltax" type="number" id="totaltaxAmount" class="form-control"
                                                        value="{{ $content->quotation->total_tax ?? 0 }}" readonly required>
                                                </td>
                                                <td class="text-center">
                                                    @php
                                                        $totalAmt = $content->quotation->final_cost;
                                                        $shipping = $content->quotation->shipping_cost;
                                                        $final_cost = $totalAmt-$shipping;

                                                    @endphp
                                                    <input name="test" id="total_cost" type="number"
                                                        value="{{ $final_cost ?? 0 }}" class="form-control" readonly
                                                        required>
                                                </td>
                                                <td>
                        ...
                        <div class="row my-3 border p-2">
                            <div class="col-lg-3 col-sm-12">
                                ...
                                <input name="shipping_cost" type="number" value="{{ $content->quotation->shipping_cost ?? 0 }}"
                            ...
                            <div class="col-lg-3 col-sm-12">
                                ...
                                <input name="final_cost" type="number" class="form-control"
                                    value="{{ $content->quotation->final_cost ?? 0 }}" id="grandtotal" readonly>
                            </div>
                            ...
                            @if (!empty($content->taxTypeIds))
                                ...
                                @endforeach
                            @endif <span class="total_tax d-none">{{ $content->quotation->total_tax ?? 0 }}
                            </span>
                                    ...
                                    @if (!empty($content->paymentterms))
                                        @foreach ($content->paymentterms as $ls_data)
                                            <option value="{{ $ls_data->terms_id }}"
                                                {{ $content->quotation->payment_term_id == $ls_data->terms_id ? 'selected' : '' }}>
                                                {{ $ls_data->terms_name }}
                                            </option>
                                        @endforeach
                                    @endif
                            ...
                            <div class="col-lg-3 col-sm-12 pt-3 ps-4" id="paymentTerm"> 
                                @php
                                    $payment_component = json_decode($content->quotation->payment_component); 
                                @endphp
                                <h5 class="mt-4"> Advance : {{$payment_component->Term->advance->amount ?? 0}}  Balance : {{$payment_component->Term->balance->amount}} </h5>
                            </div>

                           

                            <input type="hidden" name="total_tax" value="{{ $content->quotation->total_tax ?? 0 }}" id="total_tax">


                            <div class="col-lg-12 col-sm-12 pt-3 ps-4">
                                <label for="country" class="form-label">Details</label>
                                <textarea name="details" rows="2" class="form-control">{{ $content->quotation->details ?? '' }}</textarea>
                                <div class="invalid-feedback">
                                    {{ __('crm.select_customer') }}
                                </div>
                            </div>
                    ...
                    @else
                            ...
                            <div class="tab-content">
                                <div class="card-header d-md-flex d-lg-flex align-items-center justify-content-between py-2 px-3">
                                    ...
                                    {{-- <div class="dropdown_search w-50">
                                        <form class="needs-validation" id="searchform" novalidate>
                                            <input placeholder="{{ __('crm.search_item_placeholder') }}"
                                                class="form-control searchservicestext" name="searchservicestext" type="text"
                                                autocomplete="off" />
                                        </form>
                                    </div> --}}
                                    <div class="mt-2 mt-lg-0 fw-bold">OR</div>
                                    <div class="mt-2 mt-lg-0">
                                        <a href="javascript:void(0)" class="fw-bold addServicebuttom" id="addServicebuttom">
                                            <button type="button" class="btn btn-md  btn-primary "><svg
                                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                                    stroke-linejoin="round" class="feather feather-plus mg-r-5">
                                    ...
                                    <table class="table table_wrapper create_table">
                                        ...
                                        <tbody id="add">
                                                ...
                                                @foreach ($content->quotation->crm_quotation_item as $key => $quoteItem)
                                                    <tr class="addmore">
                                                        ...
                                                        <input name="quantity[]" type="hidden" class="form-control quantity"
                                                            id="quantity_{{ $key + 1 }}" placeholder="{{ __('crm.qty') }}"
                                                        ...
                                                        <input type="hidden" value="{{ json_encode($content->TaxGroup) }}"
                                                            id="dbtaxGroupData">
                                                        <input type="hidden" value="1"
                                                            id="min_prod_qty_1456823664{{ $key + 1 }}">
                                                        <input type="hidden" value='{{ json_encode(0) }}' data="json"
                                                            id="bulk_price_{{ $key + 1 }}">
                                                        ...
                                                        <td>
                                                            <input name="taxable_amount[]" type="text" class="form-control"
                                                                id="taxable_amount_{{ $key + 1 }}" onkeyup=""
                                                                placeholder="  {{ __('crm.taxable_amount_place') }}"
                                                                value="{{ $quoteItem->item_cost }}">
                                                        </td>


                                                        <td class="text-center tax_outer">
                                                            <select class="form-select form-control tax selectsearch tax_dropdown"
                                                                onchange="calculation({{ $key + 1 }})" name="tax_group_id[]"
                                                                id="tax_{{ $key + 1 }}">
                                                                <option selected value="">
                                                                    {{ __('crm.select') }}
                                                                </option>
                                                            ...
                                                            @endphp 
                                                            <div id="taxcomponent_{{$key + 1}}"> 
                                                                <div class="d-flex gap-1 justify-content-between">
                                                                        <span class="tx-11">{{$tax_string ?? ''}}</span>
                                                                </div> 
                                                            </div>
                                                        ...
                                                        <td class="text-center">
                                                            <input name="item_cost[]" type="text" class="form-control amount"
                                                                id="amount_{{ $key + 1 }}"
                                                                value="{{ $quoteItem->final_amount ?? 0 }}"
                                                                placeholder="{{ __('crm.amount') }}" value="" readonly required>
                                                            <div class="invalid-feedback">
                                                                {{ __('crm.amount_error') }}
                                                            </div>
                                                        ...
                                                        <td>
                                                            <button type="button" class="btn btn-danger btn-sm remove"
                                                                value="{{ $key + 1 }}"><svg viewBox="0 0 24 24" width="24"
                                            ...
                                            @else
                                                <tr class="addmore">
                                                    ...
                                                    <td>
                                                        <input name="discount[]" type="hidden" class="form-control discount"
                                                    ...
                                                    <td class="text-center tax_outer">
                                                        <select class="form-select form-control tax selectsearch tax_dropdown"
                                                            onchange="calculation(1)" style="width:100px" name="tax_group_id[]"
                                                            id="tax_1">
                                                            <option selected value="">{{ __('crm.select') }}
                                                            </option>
                                                            @if (!empty($TaxGroup))
                                                    ...
                                                    <td class="text-center">
                                                        <input name="item_cost[]" type="text" onkeyup="calculation(1)"
                                                            class="form-control amount" id="unit_price_1"
                                                            placeholder="{{ __('crm.amount') }}" value="" required>
                                                        <div class="invalid-feedback">
                                                            {{ __('crm.amount_error') }}
                                                        </div>
                                                    </td>
                                        ...
                                        <tbody class="border-top">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <input name="total_Descount_price" type="number" id="total_Descount"
                                                        class="form-control" value="{{ $content->quotation->discount }}" readonly
                                                        required>
                                                </td>
                                                <td>
                                                    <input name="total_taxable_amount" id="total_tax_amount" type="number"
                                                ...
                                                <td class="text-center">
                                                    <input name="total_cost" id="total_cost" type="number"
                                                        value="{{ $content->quotation->final_cost ?? 0 }}" class="form-control"
                                                        readonly required>
                                                </td>
                                                <td>
                        ...
                        <div class="row mt-4">
                            ...
                            <div class="col-lg-6 col-sm-12 pt-3 ps-4">
                                <input name="test" type="hidden" class="form-control addsub_total" id="sub_total" placeholder=""
                                    value="{{ $content->quotation->subtotal }}" required>
                                <input name="total_discount" type="hidden", class="form-control totalDiscount" onkeyup="totalDisc()"
                                    value="{{ $content->quotation->discount ?? 0 }}">
                                <input name="shipping_cost" type="hidden" value="{{ $content->quotation->shipping_cost ?? 0 }}"
                                    class="form-control" id="shipping" onkeyup="ship_disc()">
                                @if (!empty($content->taxTypeIds))
                                    @foreach ($content->taxTypeIds as $taxTypeId)
                                        <span class="d-none" id='{{ 'tax_type_ID_XYD' . $taxTypeId }}'
                                    ...
                                    @endforeach
                                @endif
                                <span class="total_tax d-none">{{ $content->quotation->total_tax ?? 0 }}
                                </span>
                                ...
                                <input type="hidden" name="total_tax" value="{{ $content->quotation->total_tax ?? 0 }}"
                                    id="total_tax">
                            </div>
    ...
    <input type="hidden" value="{{ count((array) $content->quotation->crm_quotation_item) }}" id="quoute_items">
        ...
        <script type="text/javascript">
                    ...
                    onSelect: function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $("#expiry_date").val(response.dueDate)
                                    ...
                                    <td colspan="12">
                                        ...
                                        <table>
                                            <tr>
                                                ...
                                                <td class="text-center ">
                                                    <input name="quantity[]" style="width:100px" type="number"
                                                ...
                                                <td class="text-center ">
                                                    ...
                                                    <input type="hidden" value="${response.product_qty}"  id="min_prod_qty_${product_id}">
                                                ...
                                                <td>
                                                    <input name="taxable_amount[]" type="text"  class="form-control taxable_amount" id="taxable_amount_${product_id}" value="${response.sale_price}" onkeyup="" placeholder="  {{ __('crm.taxable_amount_place') }}">
                                                </td>
                                                <td class="text-center tax_outer">    
                            ...
                            if (response != '') {
                                ...
                                html += ` 
                                                <tr class="addmore">
                                                    ...
                                                    <td>
                                                        <input name="discount[]" type="text" class="form-control discount" id="discount_${product_id}"  onkeyup="calculation(${product_id})" placeholder="{{ __('crm.discount') }}" value="0">
                                                    </td>
    
                                                    <td>
                                                        <input name="taxable_amount[]" type="text" class="form-control taxable_amount"
                                                            id="taxable_amount_${product_id}" onkeyup=""
                                                    ...
                                                    <td class="text-center tax_outer">    
                                                            ...
                                                            <div id="taxcomponent_${product_id}">${htmlData}</div>
                                                    ...
                                                    <td class="text-center ">
                                                    <input name="item_cost[]" type="number" class="form-control amount" id="amount_${product_id}" placeholder="{{ __('crm.amount') }}" value="${item_price}" readonly required>
                                                        <div class="invalid-feedback">
                                                            {{ __('crm.amount_error') }}
                                                        </div>
                                                    </td>
                    ...
                    $.each(DBTax_grp_data, function(index, DBTax_grp) {
                        if (tax_grp_id == DBTax_grp.tax_group_id) {
                            ...
                            $.each(DBTax_grp.tax_info, function(index, DBInfo) {
                                ...
                                // }
                                html +=
                                    `<span class="tx-11">${DBInfo.tax_name} : ${tax_per.toFixed(2)}</span>`;
                            });
                            html += `</div>`;
            ...
            function calculate_sub_total() {
                ...
                $('.addmore').each(function() {
                    ...
                    console.log("Tax is" + tax_data.final_price);
                    if ($('#price_enabled').find(":selected").attr('value') == "1") {
                    ...
                    if ($('#price_enabled').find(":selected").attr('value') == "1") {
                        ...
                        $('#total_tax_amount').val(parseFloat(sum - totalTax).toFixed(3));
            ...
            function ship_disc() {
                ...
                // var sub_total = $('#sub_total').val();
                var sub_total = $('#total_tax_amount').val();
                var discount = $('.totalDiscount').val();
                var grand_total = 0;
