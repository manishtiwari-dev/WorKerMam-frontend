        ...
        @if (!empty($content->quotation_list))
            @foreach ($content->quotation_list as $key => $quotation)
                <tr>
                    ...
                    <select class="form-select form-control change_status" id="customSwitch" name="status_name"  data-id="{{$quotation->id}}">
                    ...
                    <a href="{{ url('sales/quotation/details/' . $quotation->id) }}"
                            value=""class="btn btn-sm btn-white d-flex align-items-center mg-r-5">
                            <i class="fa fa-eye"></i>
                        </a>
                           @if (\App\Helper\Helper::CheckPermission($pageAccess, 'update') == 'true') 
                        <a href="{{ route('sales.quotation.edit', $quotation->id) }}" class="btn btn-sm btn-white table_btn py-1 px-2">
