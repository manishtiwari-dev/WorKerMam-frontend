      ...
      <body>
        <header class="clearfix">
          <div id="logo" style="width:60%">
            <img src="{{$data->invoice_data->invoice_logo ?? ''}}" style="margin-bottom: .5rem;" alt="{{$data->quote_detail->quotation_no ?? ''}}"><br>
          ...
          <div id="details" class="clearfix">
            ...
            <div id="address">
              ...
              <div> {{$data->quote_detail->crm_quotation_address->customer_name}},<br>
                ...
                {{$data->quote_detail->crm_quotation_address->city ?? '' }}, {{$data->quote_detail->crm_quotation_address->state ?? ''}} 
            ...
            @endif
            <div id="invoice">
              <div style="margin-bottom: 4px;"> <span style="text-align:left; font-weight: 600">Quotation No: </span> {{$data->quote_detail->quotation_no}}</div>
              <div> <span style="font-weight: 600">Date: </span>{{ \Carbon\Carbon::parse($data->quote_detail->created_at)->format('d-m-Y')}}</div>
            </div>
          </div>
          ...
          <table border="0" cellspacing="0" cellpadding="0" style="margin-top: 22px;">
            <thead>
              <tr>
                ...
                <th class="price" style="text-align: center;width:20%;">{{ __('sales.qty') }}</th>
                <th class="total" style="text-align:center;">{{ __('sales.unit_price') }}</th>
                @foreach ($data->quote_detail->crm_quotation_item as $key => $item)
               ...
               @if($data->quote_detail->tax_type==1)
                @foreach ($data->quote_detail->crm_quotation_item as $key => $item)
                    @if(!empty($item->tax_group_id))
                        <th styel="text-align:center;">{{ __('sales.tax') }} %</th>
                    @break
                   @endif
                @endforeach
               @endif
            ...
            <tbody>
              @if (!empty($data->quote_detail->crm_quotation_item))
                @foreach ($data->quote_detail->crm_quotation_item as $key => $item)
                @php
                ...
                @endphp
                  <tr>
                    ...
                    <td class="price" style="text-align: center;">{{$data->quote_detail->currency_symbol}} {{ $item->unit_price ?? '' }}</td>
                    @if(!empty($item->discount))
                        <td class="text-center">{{ $item->discount ?? '' }}</td>
                    @endif
                    @if($data->quote_detail->tax_type==1 && !empty($item->tax_group_name))
                      <td styel="text-align:center;">{{$item->tax_group_name ?? ''}}</td>
                    @endif
                    <td class="tx-center total">{{$data->quote_detail->currency_symbol}} {{ $item->item_cost ?? ''}}</td>
                    {{-- <td class="total" style="text-align:right">$1200</td> --}}
          ...
          <table>
            <thead style="border-top: 1px solid #f0f0f0;text-align: right;">
              <tr>
                ...
                <td style="text-align:right">{{$data->quote_detail->currency_symbol}} {{$data->quote_detail->subtotal ?? ''}}</td>
              </tr>
              <tr>
                <td style="width:50%; border:none;">&nbsp;</td>
                <td>{{ __('crm.total_tax') }}</td>
                <td style="text-align:right">{{$data->quote_detail->currency_symbol}} {{$data->quote_detail->total_tax ?? ''}}</td>
              </tr>
              ...
              <tr>
                ...
                <td style="text-align:right">{{$data->quote_detail->currency_symbol}} {{$data->quote_detail->final_cost ?? ''}}</td>
              </tr>
            </thead>
          </table>
          </div>
